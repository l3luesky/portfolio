...
<body>
                ...
                <c:if test="${!empty session}">
                	<div class="nav-item dropdown">
                    	...
                    	<div class="dropdown-menu fade-up m-0">
                        	...
                        	<a href="contactList.do?mem_num=${session.mem_num}" class="dropdown-item active">문의 내역</a>
                    	</div>
                	</div>
                </c:if>
            </div>
    ...
    <!-- Page Header End -->
     ...
     <input type="hidden" value="${session.mem_id}">
     <div class="container">
     <c:choose>
	     <c:when test="${session.mem_id.equalsIgnoreCase('admin@admin.com')}">
	     	<table class="table table-bordered table-hover">
	    		...
	    		<c:forEach var="vo" items="${conList}">
		    		<tr>
		    			<td><a href="goReply.do?con_num=${vo.con_num}">${vo.con_title}</a></td>
		    			<td>${vo.con_date}</td>
		    			<td>${vo.con_replyCheck}</td>
		    			<td><a href="goReply.do?con_num=${vo.con_num}" >답변</a></td>
		    		</tr>
	    		</c:forEach>
	    	</table>
	     </c:when>
	     <c:otherwise>
	     	<table class="table table-bordered table-hover">
	    		...
	    		<c:forEach var="vo" items="${conList}">
		    		<tr>
		    			<td><a href="contactView.do?con_num=${vo.con_num}">${vo.con_title}</a></td>
		    			<td>${vo.con_date}</td>
		    			<td>${vo.con_replyCheck}</td>
		    		</tr>
	    		</c:forEach>
